...
class Cli(unittest.TestCase):
    ...
    tests = [test for test in os.listdir(tests_path) if test and test.split("-", maxsplit=1)[0].isnumeric()]
    ...
    def test(self, prev_test: str | None, test: str):
        if Path(test).name.split("-", maxsplit=2)[1] == "SKIP":
            raise SkipTest("Test marked as skipped")
        if Cli.fail_fast:
